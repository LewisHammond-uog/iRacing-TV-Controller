...
$pluginProjectFile = "$basePath\StreamDeckPlugin.csproj"
...
If (-not (Test-Path $bindir)) {
  Write-Error "The output directory `"$bindir`" was not found.`n You must first build the `"StreamDeckPlugin`" project before calling this script.";
  exit 1;
}
